...
import { Check, Truck, Package, ShoppingBag, Trash2, RefreshCw, Eye, Star, Calendar, CreditCard, Tag, Percent } from 'lucide-react';
...
const OrdersPage = () => {
  ...
  const handleMaxRetriesReached = () => {
    toast.error("Impossible de charger les commandes après plusieurs tentatives.");
  };
  ...
  const canDeleteOrder = (order: any) => order.status === 'confirmée';
  const canRequestRefund = (order: any) => ['en préparation', 'en livraison', 'livrée'].includes(order.status);
  const getOrderRemboursement = (orderId: string) => userRemboursements.find(r => r.orderId === orderId);
    ...
    <Layout>
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50">
        <div className="max-w-6xl mx-auto p-4 py-8">
          ...
          >
            {/* ✅ Ici tu n’as plus besoin de gérer loadingOrders, PageDataLoader s’en charge */}
            {orders.length > 0 ? (
