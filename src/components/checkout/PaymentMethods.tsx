    ...
    <div className="bg-white p-6 rounded-lg shadow">
      ...
      <RadioGroup value={selectedMethod} onValueChange={onMethodChange} disabled={isDisabled}>
        <div className="flex flex-col space-y-3">
          ...
          {paymentModes?.enableBankTransfer && (
            <div className={`flex items-center space-x-2 rounded-md border p-3 ${selectedMethod === 'bank' ? 'border-primary bg-primary/5' : ''}`}>
              <RadioGroupItem value="bank" id="bank" />
              <Label htmlFor="bank" className="flex-grow cursor-pointer flex items-center">
                <CreditCard className="mr-2 h-4 w-4" />
      ...
      {selectedMethod === 'bank' && (
        <div className="mt-4 text-sm text-muted-foreground">
          <p>Vous recevrez les informations bancaires pour effectuer le virement après confirmation de commande.</p>
        </div>
      )}
    </div>
  );
