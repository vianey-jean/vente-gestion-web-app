...
const TacheCalendar: React.FC<TacheCalendarProps> = ({
  ...
  const handleDragOver = (e: React.DragEvent, day: number) => {
    e.preventDefault();
    setDragOverDate(getDateStr(day));
  };
  ...
  const handleDrop = (e: React.DragEvent, day: number) => {
    ...
    if (tacheId) {
      onDragTache(tacheId, getDateStr(day));
    }
        ...
        {days.map((day, idx) => {
          ...
          return (
            <div
              ...
              className={cn(
                'relative p-1 sm:p-2 rounded-xl sm:rounded-2xl cursor-pointer transition-all duration-200 min-h-[48px] sm:min-h-[64px] flex flex-col items-center justify-start hover:scale-105',
                isToday
                  ? 'bg-gradient-to-br from-violet-500/20 to-purple-500/20 border-2 border-violet-500/40 shadow-lg shadow-violet-500/20'
                  : 'bg-white/40 dark:bg-white/5 border border-white/10 hover:bg-white/60 dark:hover:bg-white/10',
                isDragOver && 'ring-2 ring-violet-500 bg-violet-500/10'
              )}
            >
              <span className={cn('text-xs sm:text-sm font-bold', isToday ? 'text-violet-600 dark:text-violet-400' : '')}>
                {day}
              </span>
              {dayTaches.length > 0 && (
